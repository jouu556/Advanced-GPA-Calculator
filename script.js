const gradePoints = {
  ...
  document.getElementById("calcGoalBtn").addEventListener("click", function() {
    ...
    const completedCredits = parseFloat(document.getElementById("goalCompletedCredits").value);
    const goalGPA = parseFloat(document.getElementById("goalGPA").value);
    const nextCredits = parseFloat(document.getElementById("nextSemesterCredits").value);
    ...
    if (
      ...
      alert("Please enter all fields correctly for the goal GPA calculation.");
      return;
    }
    ...
    const requiredSemesterGPA = (requiredTotalPoints - earnedPoints) / nextCredits;
